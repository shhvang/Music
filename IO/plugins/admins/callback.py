...
from IO.utils.formatters import seconds_to_min
from IO.utils.inline import close_markup, stream_markup, stream_markup_timer
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        if command == "Skip":
            txt = f"Stream skipped by User : {mention}"
            popped = None
            try:
                popped = check.pop(0)
                if popped:
                ...
                if not check:
                    await CallbackQuery.edit_message_text(
                        f"Stream skipped by User : {mention}"
                    )
                    await CallbackQuery.message.reply_text(
            ...
            txt = f"Stream replayed by User : {mention}"
        await CallbackQuery.answer()
        queued = check[0]["file"]
